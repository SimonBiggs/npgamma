...
import json
import numpy as np
from npgamma import GammaCalculation
...
class RunNpgamma(tornado.web.RequestHandler):
    ...
    def get(self):
        ...
        coords = (grid, grid, grid)
        reference = np.zeros(dimensions)
        reference[2:-2:, 2:-2:, 2:-2:] = 1

        evaluation = np.zeros(dimensions)
        evaluation[3:-2:, 4:-2:, 5:-2:] = 1.015
        ...
        self.write(json.dumps(gamma_calculation.gamma.tolist()))
