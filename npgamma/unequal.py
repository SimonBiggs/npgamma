...
def calc_gamma_1d(x_reference, dose_reference,
                  x_evaluation, dose_evaluation,
                  dose_threshold=None, distance_threshold=None):

    xref = np.array(x_reference)
    dose_ref = np.array(dose_reference)

    xeval = np.array(x_evaluation)
    dose_eval = np.array(dose_evaluation)
